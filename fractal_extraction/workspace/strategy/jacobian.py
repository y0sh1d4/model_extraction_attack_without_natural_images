...
class attack(strategy_base):
    def __init__(self, initial_samples, max_samples, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.ds_ext = iter(self.ds_ext)
        self.ds = extraction_dataset(initial_samples, max_samples)
    ...
    def update(self, query, response):
        ...
        self.model = self.train(self.model, self.ds.dataset(self.cfg))
        acc = self.test()
        return acc
    ...
    def train(self, model, dataset):
        ...
        return substitute_model.layers[0]

    def test(self):
        # Test
        predicts = self.model.predict(self.ds_test).argmax(axis=1)
        labels = np.concatenate([v[1].numpy() for v in self.ds_test], axis=0)
